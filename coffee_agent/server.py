...
from .settings import DOCS_DIR, Settings, load_settings, openai_base
...
def main() -> None:
    ...
    exe_ok = Path(settings.exe).exists()
    print("=" * 64)
    print("  瑞幸点单 Agent · 桥接服务")
    print("=" * 64)
    print(f"  luckin CLI : {settings.exe}  {'OK' if exe_ok else '❌ 未找到'}")
    print(f"  服务端模型 : {settings.model if settings.llm_ready else '❌ 未配置'}  ({settings.model_source})")
    print(f"  静态站点   : {DOCS_DIR}")
    ...
    if settings.cross_origin:
        ...
        if settings.allow_any_origin and not settings.password:
            ...
            print("      建议同时设置 WEB_PASSWORD，或把 WEB_ALLOW_ORIGINS 收敛到具体域名。")
    print("=" * 64)
